...
**Known issue:** нода сейчас обращается к parent-node `Загрузить факты закупки`; см. `TR-3`.
...
**FALSE:** финализация `not_found`.

Текущая FALSE-ветка корректна только для сценария без старого candidate. См. `TR-0`.
...
**FALSE:** all candidates rejected / unusable.

Текущая FALSE-ветка корректно завершает `not_found`, только если до Recheck не существовало usable existing candidate. См. `TR-1`.
...
**Known issue:** текущий `FIELD_RULES` внутри этой ноды содержит только часть 27 полей. См. `TR-2`.
...
| IF / route | TRUE | FALSE |
|---|---|---|
| `Контекст для Targeted Recheck найден?` | AI Extractor | `not_found` |
...
| `Есть пригодные candidates после Validator?` | route selection | all rejected / `not_found` |
| `Путь = direct_final?` | direct `resolved` | Semantic Aggregator Round 2 |

Важно: две FALSE-ветки сейчас имеют известные edge-case gaps — см. `TR-0` и `TR-1`.
...
| no context + existing candidate | ❌ не закрыто | `TR-0` |
| новые candidates all rejected + existing candidate | ❌ не закрыто | `TR-1` |
| Round 2 для field_key вне текущих FIELD_RULES | ❌ не закрыто | `TR-2` |
...
### TR-0 — `context_found=false` при наличии исходного candidate
...
FALSE-ветка первого IF идёт напрямую в `not_found Finalizer`.

`not_found Finalizer` корректно запрещает `not_found`, потому что:

```text
original_aggregation.status != no_initial_candidates
```

В результате execution падает.
...
Отсутствие нового контекста не должно уничтожать исходный candidate.

---

### TR-1 — все новые Recheck candidates rejected, но исходный candidate существует
...
existing candidate есть
        |
Targeted Recheck нашёл новый candidate
        |
Independent Validator отклонил все новые candidates
```

#### Текущее поведение

FALSE `Есть пригодные candidates после Validator?` идёт в `not_found Finalizer`.

Guard не позволяет поставить ложный `not_found`, поэтому execution падает.

#### Требуемый смысл

```text
новые candidates отклонены
+
старый candidate остаётся
=
requires_review
```
...
### TR-2 — Semantic Aggregator Round 2 не поддерживает все 27 полей

**Severity:** Critical  
**Status:** Open

`RECHECK_PROFILES` уже содержит 27 полей.

Но `FIELD_RULES` в `Подготовить запрос #2 Semantic Aggregator` Round 2 в текущей реализации содержит только часть полей, в частности были подтверждены:
...
Если другой `field_key` попадёт в Round 2:
...
Требуется синхронизировать Round 2 field rules со всем каталогом 27 полей.

---

### TR-3 — ссылка на parent node `Загрузить факты закупки`

**Severity:** Critical / architectural  
**Status:** Open

В `Подготовить #2 Targeted Recheck Request` используется:
...
Но `Загрузить факты закупки` находится в родительском workflow:
...
RAW input sub-workflow сейчас `tender_meta` явно не содержит.

Необходимо убрать скрытую parent-reference зависимость.

Возможные будущие решения:

1. parent явно передаёт `tender_meta`;
2. child получает необходимую TenderMeta сам по `analysis_run_id`.
...
Ожидание после исправления `TR-0`:
...
Ожидание после исправления `TR-1`:
...
Ожидание после исправления `TR-2`:

```text
нет ошибки "Пока нет правил для field_key"
```
...
1. `TR-0` — no context + existing candidate.
2. `TR-1` — all new rejected + existing candidate.
3. `TR-2` — Round 2 rules на все 27 полей.
4. `TR-3` — удалить parent-node reference.
5. Прогнать regression checklist.
6. `TR-6` — selective AI retries.
7. `TR-7` — убрать случайный fallback `existingCandidates[0]`.
8. `TR-5` — вынести Normalize + UPSERT в общий persist sub-workflow.
9. `TR-8` — централизовать versioned field rules при стабилизации MVP.
10. `TR-9` — улучшать retrieval только по результатам реальных miss cases.
