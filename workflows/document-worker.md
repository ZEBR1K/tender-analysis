...
Exact read-only replay полного execution `14359` input связал activeX5/6/7/8 только с `#/tables/2` и activeX55/56 только с `#/tables/25`, сохранив состояния `0/0/0/1` и `1/0`. Общий `docx_option_state_semantic_status` при этом остаётся `unknown`: `33` warning groups, semantic owners `83/290`. В local candidate этот global audit status больше не понижает независимо resolved target group; affected target group всё ещё fail-closed остаётся `requires_review`. Это diagnostic replay и offline regression basis, не production runtime GREEN; production n8n/DB/credentials не изменялись.
