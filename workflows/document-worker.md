...
## 9A. DOCX ActiveX option-state and structural ownership boundary (`DW-18`, `DW-21`)
...
Semantic owner после Docling определяется отдельно и структурно. Parser сохраняет для каждого control typed refs `source_table_ref`, `source_row_ref`, `source_control_cell_ref`, `source_label_cell_ref`, ancestry таблиц и только один допустимый question candidate с relation `direct_enclosing_cell_child`. Preceding paragraph/row не выводится из proximity: candidate существует только как непосредственный OOXML child enclosing cell и только при `candidate_count=1`. Controls группируются по строгому source table; один normalized table должен содержать всю группу с единым relative row offset. Это сохраняет relative row geometry при wrapper/nested shifts и не приравнивает OOXML table ordinal к Docling ordinal. Question candidate затем должен совпасть ровно с одним normalized source block и ровно с одним semantic block. Missing coordinate, conflicting stable control identity, duplicate source row, ноль или несколько structural/semantic owner matches дают fail-closed mapping issue. Byte offsets, control names, ActiveX part numbers, fixed block/table IDs, fixture labels и tender-specific literals не используются как production mapping rules.

После нормализации canonical block text и evidence quote остаются неизменными. Additive `docx_option_state_v1` metadata отдельно несёт exact state/label, source table-row-cell refs, `option_group_id`, observed mutual-exclusion status, exact question owner, semantic owner block ID, `mapping_status` и bounded audit. AI-visible marker показывает `selected`/`unselected`, group и owner только для однозначно mapped state; unresolved/multiple owner получает только `OPTION_MAPPING review_only` и не представляется модели как applicable.

Terminal boundary в `Подготовить dispatch AI Validator` универсален и fact-local: он не содержит списка field keys и не применяет document-wide blanket cap. Источник связи — только grounded `evidence[].quote`, не `value_text`. Полная normalized line/cell equality принимается напрямую; допустимая короткая непрерывная quote сопоставляется с attached exact label только по Unicode lexical boundaries. Проверяются все structurally attached rows: один mapped selected owner остаётся eligible для обычного Validator, один mapped unselected становится audited deterministic rejection, а multiple states/groups/owners, unknown/indeterminate или mapping issue получают `requires_review` и `accepted_for_normalization=false`. Primary, fallback, Evidence Repair attempt 2, partition/dispatch и post-AI paths сохраняют proof; `Проверить ответ AI Validator` не позволяет AI повысить review-only в confirmed и сохраняет `original_ai_verdict`, evidence и `option_state_audit`. Persistence переносит audit в `validator_meta.option_state`; AI не создаёт provenance или option-state facts.
...
`DW-21` is locally GREEN on an execution-derived sanitized structural projection. The fixture preserves relationship identity, nested table/row/cell ancestry, relative geometry and duplicate-label collision topology without client text or a full DOCX. Focused `19/19` proves the original exact-grounding/applicability boundary, real parser derivation, unique owner mapping, selected eligibility, unselected exclusion, nested tables, duplicate labels in separate groups, missing/multiple owners, conflicting coordinates, shared partial quotes and neutral facts. Non-isolated Worker is `255/252/3`; full repository suite is `423/416/7`, with exactly the pre-existing signatures and no new failures. Detailed source audit remains `evaluations/DOCUMENT_WORKER_SEMANTIC_AUDIT_14374_14376_2026-09-03.md`. Runtime/promotion verification is still pending and requires a separate reviewed checkpoint.
