...
Normalizer добавляет к owner block versioned container `docx_option_state_semantic_v2`. Persisted control identity кодируется как `'v2i:' + JSON.stringify([document_part, control_rel_target])`; question identity — как `'v2q:' + JSON.stringify([source_table_ref, source_row_index, normalized_question_label])`, где `source_row_index` — фактическая 1-based integer structural coordinate (`>= 1`); в zero-based индекс она преобразуется только при обращении к массивам; public group identity — как `'v2g:' + JSON.stringify([owner_block_id, source_table_ref, control_type, group_discriminator])`. Исходный `source_row_ref` сохраняется отдельно без изменения для audit. Exact trimmed `GroupName` сохраняется как discriminator, а для unnamed structurally owned options используется normalized question label. Versioned identities проходят через option records, semantic blocks и AI segments. Execution-level tests реальной Normalizer-ноды подтверждают tuple members, collision-safe encoding и рекурсивное отсутствие literal U+0000. Existing group-local owner guards, rejected alternatives, fail-closed mapping audit и post-Validator caps сохраняются.
