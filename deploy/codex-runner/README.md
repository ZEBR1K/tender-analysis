...
The runner builds a fresh permission profile for each job and supplies it through CLI `-c` overrides after `--ignore-user-config`. It never passes the legacy `--sandbox` flag because current Codex permission profiles and the legacy sandbox do not compose. The profile denies the filesystem root by default, restores only `:minimal` read access, writes only the exact current `workspace` (including its job-local `.tmp`), reads only that job's immutable original `input`, and explicitly denies both Codex auth locations, runner secrets, global `/tmp` and the complete `/proc` tree. The inner sandbox exposes only the synthetic ancestor path to the current job; the canary verifies that no sibling or unrelated job entry is visible. No generated source-index tree is mounted or granted.
...
This is a bounded, real Codex call and may consume paid usage. It creates two fresh canary jobs, runs the image-owned hash-pinned probe under the same model, reasoning effort, permission profile, environment policy and no-network controls as production execution, and accepts only Codex JSONL command-execution evidence. Agent prose or an agent-written claim cannot attest the runner. The probe must prove current-input read, workspace and job-local `.tmp` write, current-input write denial, sibling/job-parent denial, auth and runner-secret denial, global `/tmp` write denial, self/parent environment denial, and absence of credential-like environment names.
