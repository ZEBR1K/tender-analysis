...
|`DW-22`|⚠ Corrective offline GREEN / promotion+runtime pending. Execution `14487` failed in `Подготовить Evidence Repair` at candidate `257`. Independent sanitized inspection disproved the first exact-whole-anchor fixture: observed geometry is `44,375` canonical chars / `2,016` candidates / `2,015` primary `sb_0714` candidates, with no candidate equal to normalized value/invalid quote; real lexical-overlap scores and ranked ordinals were not captured. Canonical local Worker preserves byte-compatible `<=256` full mode and uses bounded deterministic token overlap only for exactly one invalid fact / one repaired-fact context. Multi-fact overflow is intentionally hard-failed before ranking because the merged catalog lacks fact-local relevance and can expose a primary candidate for the wrong fact; support remains blocked on a future fact-local catalog contract. Strict materialization/validation and attempt-2 parity remain unchanged. The distributed late-fragment fixture is synthetic algorithmic coverage. Zero/ambiguous overlap, absent target/primary and budget cases fail closed; focused actual-jsCode GREEN, no live change. Runtime token/window coverage remains a canary gate.|Document Worker / Evidence Repair catalog bounding|
