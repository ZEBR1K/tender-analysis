...
|`DW-23`|⚠ Local GREEN / promotion+runtime pending. Execution `14491` failed in `Проверить ответ AI Validator` for `doc_7_au_0037#1 / licenses_certificates` because `confidence` was absent. Canonical local Worker now keeps the primary Validator outside retry, classifies exact `analysis_unit_id + fact_index + field_key`, and sends only contract-invalid facts through at most two additional calls; valid siblings/units and semantic verdicts are not retried. Exhaustion becomes deterministic `requires_review` with `confidence=0` system sentinel, `reason_code=other`, original value/evidence and complete three-attempt audit before the unchanged strict final checker. Execution-derived sanitized focused tests `6/6` and related regression parity are GREEN; production promotion/read-back and fresh runtime canary through persistence/completion remain pending.|Document Worker / AI reliability and terminal containment|
