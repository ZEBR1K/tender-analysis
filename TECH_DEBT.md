...
|`DW-19`|⚠ Local GREEN, runtime canary pending. Execution `14359` дал `0/16` safely attachable primary Extractor responses из-за model contract noncompliance при неизменном exact prompt/version contract. Execution `14362` также выявил latency regression: Loop перед primary дал `12` последовательных node runs / `423` s против одного `16`-item node run / ~`32` s в `14359`. Canonical local Worker снова запускает primary multi-item до Loop (`batchSize=16`, interval `1000`, timeout `180000`), затем сериализует только decisions/convergence и bounded fallback/evidence retries. Семь-code allow-list, максимум один Gemini fallback, shared strict validator, explicit source envelope и done-only barrier сохранены. Production не изменён.|Document Worker / Extractor reliability and latency|
...
Primary and fallback HTTP nodes hard-stop transport errors, do not retry internally and expose no error output. Primary runs as a multi-item node before Loop with batching `16 / 1000 ms` and timeout `180000`; a regular-success wrapper resolves paired lineage to create an explicit source/attempt/provider-response envelope before classification. A local non-identity 16-item permutation proves wrapper behavior when `pairedItem` is present; installed-runtime propagation of linked items across the batched HTTP node remains pending. The primary classifier returns only `accepted` or `fallback_required` for the exact seven-code allow-list; unknown/internal/source/envelope defects hard-stop. Fallback uses the original prompts/response contract and Gemini 3.7 Flash low once, then passes the same byte-identical strict validator body; invalid fallback hard-stops the document. Accepted primary items never call fallback.
...
The batch-first latency recovery has local TDD coverage for topology, 16-item paired mapping, order/cardinality, invalid-subset-only fallback, maximum one fallback, done-only persistence reachability, hard-stop transport and request metrics. No production n8n/PostgreSQL writes, credential changes or paid AI calls occurred. Runtime canary remains required; Stage 2 is not runtime complete.
