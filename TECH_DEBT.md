...
|`DW-19`|⚠ Local GREEN, second runtime canary authorization pending. Execution `14359` дал `0/16` safely attachable primary responses из-за model contract noncompliance; execution `14362` выявил latency regression (`12` sequential runs / `423` s против одного `16`-item run / ~`32` s). Execution `14365` подтвердил новый configuration blocker: HTTP Request `4.4` ожидает nested `options.batching.batch`, а flat shape из `66d5a8f` hard-stop-нулся на `batchInterval`. Canonical local Worker теперь задаёт `options.batching.batch = { batchSize: 16, batchInterval: 1000 }`, timeout `180000`, primary до Loop; decisions/convergence и bounded fallback/evidence retries остаются serial `batchSize=1`. Семь-code allow-list, максимум один Gemini fallback, shared strict validator, explicit source envelope и done-only barrier сохранены. Runtime GREEN не заявлен; второй canary требует отдельного review/разрешения. Production не изменён.|Document Worker / Extractor reliability and latency|
...
The batch-first latency recovery has local TDD coverage for topology, nested HTTP batching, 16-item paired mapping, order/cardinality, invalid-subset-only fallback, maximum one fallback, done-only persistence reachability, hard-stop transport and request metrics. No production n8n/PostgreSQL writes or credential changes occurred in this local fix. Runtime canary remains required; Stage 2 is not runtime complete. A second canary is explicitly blocked pending separate owner review and authorization.
