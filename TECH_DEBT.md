...
|`OR-0`|✅ Closed for the active agentic path. Все исходные файлы регистрируются; PDF/DOCX/XLSX/XLS и раскрытые поддерживаемые элементы получают `pending`, а неподдерживаемые файлы и архивные контейнеры — terminal `skipped` с audit-причиной. Agentic Dispatch получает только подготовленные `pending` документы; сохранённая legacy Worker-ветка остаётся недостижимой.|Orchestrator / Document Preparation / Intake Resume|
...
|`RG-1`|✅ Closed 2026-09-11. Published Report Generation version `e21c7675-…` ran as integrated execution `15663` from agentic Finalization `15662`: snapshot/model `27/27`, HTML `24761` bytes valid, PDF `90367` bytes with `%PDF-`, both binaries preserved. Read-only API confirmed active/draft parity; replay `15666` did not invoke a duplicate report.|Report Generation / PDF production canary|
...
Статус остаётся **open**, но promotion-artifact gate закрыт read-only проверкой: active live workflow `ftvmrEHoMbPOAqZG` уже содержит boundary и синхронизирован в canonical export. A/B harness не обращался к БД/n8n и менял только `request.model`; fresh runtime canary и полный 27/27 run с manual semantic review всё ещё обязательны.
...
Primary and fallback HTTP nodes hard-stop transport errors, do not retry internally and expose no error output. Primary runs as a multi-item node before Loop with `options.batching.batch = { batchSize: 16, batchInterval: 1000 }` and timeout `180000`; a regular-success wrapper resolves paired lineage to create an explicit source/attempt/provider-response envelope before classification. A local non-identity 16-item permutation proves wrapper behavior, and isolated execution `14367` confirms installed-runtime `pairedItem=0..11` propagation for an ordered 12-item batch. The primary classifier returns only `accepted` or `fallback_required` for the exact seven-code allow-list; unknown/internal/source/envelope defects hard-stop. Fallback uses the original prompts/response contract and Gemini 3.7 Flash low once, then passes the same byte-identical strict validator body; invalid fallback hard-stops the document. Accepted primary items never call fallback.

Loop `batchSize=1` receives already checked primary decisions and returns only successful primary/fallback evidence tails once. Output 0 reaches a separate barrier that validates expected cardinality, unique IDs, deterministic source order and bounded attempt audit before the pre-existing collector, Validator dispatch or fact persistence. Primary HTTP is unreachable from the loop body. No Merge or error-output source reconstruction is used. Fallback `recovery_context` is an exact bounded current-input contract, is validated outside the provider semantic payload and removed when final `attempt_audit` is materialized. Metrics retain unit-compatible `extractor_requests`, count `extractor_fallback_requests` separately and include primary + fallback + evidence repair + fact partition in `total_ai_requests_before_ai_validator`.

Execution `14365` stopped in `AI Extractor v1.0` with `Cannot read properties of undefined (reading 'batchInterval')`. The installed HTTP Request `4.4` type definition exposes only `options.batching.batch.batchSize/batchInterval`; the flat `options.batching.batchSize/batchInterval` asserted by `66d5a8f` was version-incompatible. Это прямой конфликт repository config/documentation с platform source of truth; installed type definition и observed runtime имеют приоритет для parameter shape. Execution-derived RED rejects flat keys across every affected batched Extractor HTTP node, а minimal GREEN только добавляет nested `batch` без изменения значений, topology, prompts, models, credentials, retry/onError или downstream contracts.

Execution `14367` closes the bounded batch-first runtime gate on the disposable inactive/unpublished Yq clone: primary `1` run / `12` outputs / `4339 ms`, one deterministic fallback, Loop `12 + done`, barrier exact `12/12`, paid calls `13 <= 25`, and zero runs/reachability for PostgreSQL, collector, AI Validator, persistence, Aggregator, download/Docling or delivery. The clone's terminal summary and attempt audits prove observed request counts, but persistence-facing `extractor_requests`, `extractor_fallback_requests` and `total_ai_requests_before_ai_validator` were not materialized because collector is deliberately unreachable.

This is not promotion: the five-node canary contour is not a production candidate, and canonical commit `2ffe1de` has not been imported as a complete clean workflow. The next gate is separate reviewed packaging/import into a new inactive/unpublished candidate with exact read-back; only then may a separately authorized full-path Validator/persistence test be considered.
...
✅ HTML artifact → internal Gotenberg → PDF artifact (isolated and published production runtime GREEN)
future: DOCX
future: XLSX
future: manual upload / automatic delivery
```

HTML/PDF report path реализован, опубликован и прошёл production execution
`15663`; `RG-1` закрыт. Завершение полного delivery-MVP зависит от отдельных
будущих интеграций.
...
fallback-lane debt (`DW-17`–`DW-24`, `AG-8`–`AG-11`) и не требует возвращать его
в активный маршрут до результатов agentic проверки. Terminal agentic
архитектура уже runtime GREEN и не нуждается в новом parser/validator redesign.
