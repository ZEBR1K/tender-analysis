...
|`AG-9`|⚠ Offline route-guard GREEN / runtime pending: Aggregator boundary `14173/14254` сохранена. Owner-started run `14429` выявил следующий defect: validated `application_documents` получил `post_validator_route=round_2` и вызвал общий `Semantic Aggregator1`. Local Targeted Recheck теперь оставляет `direct_final` без изменений, а non-direct result terminally завершает как audited `requires_review`; общий Round 2 недостижим. Production/test n8n не менялись; runtime canary и fresh full run остаются обязательными.|Aggregator / Targeted Recheck semantic safety|
...
|`TR-10`|⚠ Offline route-guard GREEN / runtime pending: execution `14429` доказал нарушение route invariant до прежнего containment. Local canonical route разрешает общий Round 2 только для `procurement_subject`, `nm_price_with_vat`, `delivery_term`, `warranty_obligations_guarantee`; остальные 23 поля идут в audited terminal `requires_review`, а collector повторяет allow-list fail-closed. Прежний containment сохранён как defense in depth. LIVE/test workflows не изменялись.|Targeted Recheck semantic safety|
