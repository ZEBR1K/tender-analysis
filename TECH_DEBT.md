...
|`DW-24`|⚠ Local/offline GREEN; runtime pending. Execution `14592` exposed malformed ActiveX GroupName with literal U+0000 while option Value `0/1` remained decodable. Canonical and DW-23 beta packages reject only GroupName to `group_context=null`, preserve state, emit bounded NUL-free `invalid_activex_group_name`, and fail fast on any residual nested NUL before `Сохранить analysis unit`. Production/live DB unchanged; runtime replay remains required.|Document Worker / DOCX ActiveX JSONB safety|
