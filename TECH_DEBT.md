...
|`DW-23`|⚠ P0 observed AI contract failure / implementation pending. Execution `14491` later failed in `Проверить ответ AI Validator` for `doc_7_au_0037`, `licenses_certificates`, because `validations[1].confidence` was absent (`undefined`). The strict checker correctly failed closed, but one malformed AI item stopped all persistence/completion and no selective contract retry exists. Agreed change: local in-workflow retry branch (no sub-workflow), retry only contract-invalid items by stable identity, three total attempts by default, bounded waits and attempt audit; valid items are never re-sent. Semantic verdicts are not retried. After exhaustion use a stage-specific technical terminal fallback without fabricating confidence/evidence or converting failure to `not_found`. Implement first for AI Validator with an execution-`14491`-derived RED, then roll out per AI stage only after separate contract review.|Document Worker / AI reliability and terminal containment|
