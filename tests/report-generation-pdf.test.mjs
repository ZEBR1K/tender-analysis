...
test('PDF candidate preserves every canonical report node configuration', () => {
  ...
  assert.equal(canonical.nodes.length, 9);
  assert.equal(candidate.nodes.length, 12);

  for (const canonicalNode of canonical.nodes) {
    const candidateNode = byName(candidate, canonicalNode.name);
    assert.equal(candidateNode.type, canonicalNode.type, canonicalNode.name);
    assert.equal(candidateNode.typeVersion, canonicalNode.typeVersion, canonicalNode.name);
    assert.deepEqual(candidateNode.parameters, canonicalNode.parameters, canonicalNode.name);
  }

  for (const [source, connection] of Object.entries(canonical.connections)) {
    if (source === 'Создать HTML artifact') continue;
    assert.deepEqual(candidate.connections[source], connection, source);
  }
});

test('PDF candidate appends one fail-fast HTML to PDF chain', () => {
  const workflow = loadJson(candidatePath);
...
test('HTTP Request posts the validated HTML binary to the internal Gotenberg endpoint', () => {
  ...
  assert.equal(convert.parameters.authentication, 'none');
