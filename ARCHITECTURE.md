...
```text
TR-0
existing candidate + no retrieval context path broken

TR-1
existing candidate + all new rejected path broken

TR-2
Round 2 supports only 4/27 fields

TR-3
child references parent node for tender_meta
```
...
После завершения документации рекомендуемый порядок:
...
На реальном integration run подтверждено:
...
→ facts
→ document completed
→ final Worker makes run ready
→ Aggregator
→ 27 field items
→ FINAL field persistence
```

Также подтверждалось, что 27 полей могут завершаться в нескольких отдельных branch executions, поэтому centralized in-memory Merge не подходит для final synchronization.
...
После фикса critical Recheck paths:
...
Он должен стать новой системной точкой синхронизации между:

```text
fan-out 27 field processing
```

и:

```text
single final report generation
```
